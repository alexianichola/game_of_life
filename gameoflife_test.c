    ...
    for (int i = 1; i < argc; i = i + 2)
    {
        ...
        if (T == 2)
        {
            ...
            for (p = 0; p < K; p++)
            {
                ...
                diferente = generatii(N, M, stare_curenta);
                push(&stack, copie_lista(diferente));

                // aici se creeaza ce vom folosi pentru generatia urmatoare
        ...
        if (T == 3)
        {
            ...
            Tree* root = construiesteArbore(root_list, 0, N, M, K);
           

            preordine(root, matrice, N, M, output_file);
            deleteTree(root);
            deletelist(root_list);
        }
